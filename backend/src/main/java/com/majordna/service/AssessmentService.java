...
public class AssessmentService {
    ...
    public AssessmentScores scoreDetailed(Map<String,Integer> answers,String mode){
        ...
        for(Question q:selected){int raw=Math.max(1,Math.min(5,answers==null?3:answers.getOrDefault(q.id(),3)));int keyed=q.reverse()?6-raw:raw;double normalized=(keyed-1)*25.0;
